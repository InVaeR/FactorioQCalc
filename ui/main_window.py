...
from core.data import QUALITY_NAMES, MAX_QUALITY_LEVEL
...
class MainWindow(QMainWindow):
    ...
    def _make_param_row(self, label_text: str, widget, parent: QVBoxLayout):
        ...
        label.setStyleSheet("color: #a0a0a0; font-size: 13px; background: transparent;")
        row.addWidget(label)
    ...
    def _build_input_panel(self, parent: QVBoxLayout):
        ...
        self.production_rate.setToolTip("Объём производства в минуту")
        ...
        self._make_param_row("Выработка / мин:", self.production_rate, right_col)
        ...
        """)
        self.recycle_check.stateChanged.connect(self._on_recycle_toggle)
        ...
        rec_thresh_row = QHBoxLayout()
        rec_thresh_row.setSpacing(8)
        rec_thresh_label = QLabel("Порог рециклинга:")
        rec_thresh_label.setFixedWidth(150)
        rec_thresh_label.setStyleSheet("color: #a0a0a0; font-size: 13px; background: transparent;")
        rec_thresh_row.addWidget(rec_thresh_label)
        rec_thresh_row.addWidget(self.recycle_threshold, 1)
        rec_left.addLayout(rec_thresh_row)
        self._recycling_labels.append(rec_thresh_label)

        rec_mod_count_row = QHBoxLayout()
        rec_mod_count_row.setSpacing(8)
        rec_mod_count_label = QLabel("Кол-во модулей рец.:")
        rec_mod_count_label.setFixedWidth(150)
        rec_mod_count_label.setStyleSheet("color: #a0a0a0; font-size: 13px; background: transparent;")
        rec_mod_count_row.addWidget(rec_mod_count_label)
        rec_mod_count_row.addWidget(self.rec_module_count, 1)
        rec_left.addLayout(rec_mod_count_row)
        self._recycling_labels.append(rec_mod_count_label)

        rec_right = QVBoxLayout()
        rec_right.setSpacing(6)
        rec_tier_row = QHBoxLayout()
        rec_tier_row.setSpacing(8)
        rec_tier_label = QLabel("Уровень модуля рец.:")
        rec_tier_label.setFixedWidth(150)
        rec_tier_label.setStyleSheet("color: #a0a0a0; font-size: 13px; background: transparent;")
        rec_tier_row.addWidget(rec_tier_label)
        rec_tier_row.addWidget(self.rec_module_tier, 1)
        rec_right.addLayout(rec_tier_row)
        self._recycling_labels.append(rec_tier_label)

        rec_qual_row = QHBoxLayout()
        rec_qual_row.setSpacing(8)
        rec_qual_label = QLabel("Качество модуля рец.:")
        rec_qual_label.setFixedWidth(150)
        rec_qual_label.setStyleSheet("color: #a0a0a0; font-size: 13px; background: transparent;")
        rec_qual_row.addWidget(rec_qual_label)
        rec_qual_row.addWidget(self.rec_module_quality, 1)
        rec_right.addLayout(rec_qual_row)
        self._recycling_labels.append(rec_qual_label)
    ...
    def _on_recycle_toggle(self):
        QTimer.singleShot(0, self._do_toggle_recycling_fields)

    def _do_toggle_recycling_fields(self):
        enabled = self.recycle_check.isChecked()
        for w in self._recycling_widgets:
            w.setVisible(enabled)
        for w in self._recycling_labels:
            w.setVisible(enabled)

    def _toggle_recycling_fields(self, enabled):
        for w in self._recycling_widgets:
            w.setVisible(enabled)
        for w in self._recycling_labels:
            w.setVisible(enabled)
    ...
    def _build_result_panel(self, parent: QVBoxLayout):
        ...
        self.total_chance_label.setStyleSheet(
            "color: #d97b14; font-size: 16px; font-weight: bold; background: transparent;"
        )
        ...
        self.recycle_info_label.setStyleSheet(
            "color: #4a8c1c; font-size: 16px; font-weight: bold; background: transparent;"
        )
        ...
        self.table.setHorizontalHeaderLabels(["Качество", "Доля, %", "Шт./мин"])
    ...
    def _on_calculate(self):
        ...
        rate = self.production_rate.value()
        ...
        self._toggle_recycling_fields(using_recycling)
        ...
        if using_recycling:
            ...
            rec_count = self.rec_module_count.value()
            rec_tier = self.rec_module_tier.currentData()
            rec_qual = self.rec_module_quality.currentData()

            Q_prod = total_chance(count, tier, quality)
            Q_rec = total_chance(rec_count, rec_tier, rec_qual)
            ...
            loss = rate - total_out  # not exactly loss, but difference
            # Actual loss = 75% of everything that went through recycler

            self.total_chance_label.setText(
                f"Суммарный шанс качества: {Q_prod * 100:.2f}%"
            )
            self.max_quality_label.setText(
                f"Макс. открытое качество: {QUALITY_NAMES[max_unlocked]}"
            )
            self.recycle_info_label.setText(
                f"Коэфф. рециклинга: {ratio:.4f}  |  Шанс рец.: {Q_rec * 100:.2f}%"
            )
        else:
        ...
        total_input = sum(result_items.values())
        for level in range(start, max_unlocked + 1):
            ...
            icon = QIcon(QUALITY_ICONS[level])
            share_pct = (items / total_input * 100) if total_input > 0 else 0.0
            total_share += share_pct

            self.table.setItem(row, 0, QTableWidgetItem(icon, QUALITY_NAMES[level]))
            self.table.setItem(row, 1, QTableWidgetItem(f"{share_pct:.2f}"))
            self.table.setItem(row, 2, QTableWidgetItem(f"{items:.2f}"))
        ...
        total_item_0 = QTableWidgetItem("ИТОГО")
        total_item_0.setForeground(gold)
        total_item_0.setFont(bold_font)
        total_item_0.setBackground(QBrush(QColor("#1a1a1a")))
        self.table.setItem(row, 0, total_item_0)

        total_item_1 = QTableWidgetItem(f"{total_share:.2f}")
        total_item_1.setForeground(gold)
        total_item_1.setFont(bold_font)
        total_item_1.setBackground(QBrush(QColor("#1a1a1a")))
        self.table.setItem(row, 1, total_item_1)

        total_item_2 = QTableWidgetItem(f"{total_rate:.2f}")
        total_item_2.setForeground(gold)
        total_item_2.setFont(bold_font)
        total_item_2.setBackground(QBrush(QColor("#1a1a1a")))
        self.table.setItem(row, 2, total_item_2)
